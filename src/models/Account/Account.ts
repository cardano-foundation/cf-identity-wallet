...
export class Account implements IAccount {
    ...
    era: ERA | undefined;
    network: { [key: string]: { assets: { [unit: string]: IAsset }; utxos: IUtxo[]; collateral:string[]; vault:string[]; addresses: IAddress[]; transactions: ITransaction[]} };
    ...
    setCertificate(name:string, key: string, data:string, derivation=undefined) {
    ...
    address(network:string, index:number, chain: number) {
       return this.network[network].addresses.find((address) => address.index === index && address.chain === chain)
    }

    externalAddresses(network:string) {
       return this.network[network].addresses.filter(address => address.chain === 0)
    }

    internalAddresses(network:string) {
       return this.network[network].addresses.filter(address => address.chain === 1)
    }
    ...
    pendingTransactions(network:string) {
       return this.network[network].transactions.filter(tx => tx.status === TX_STATUS.PENDING)
    }
    ...
    addTransaction(network:string, tx: ITransaction) {
        ...
        const index = this.network[network].transactions.findIndex(t => t.txHash === tx.txHash);
