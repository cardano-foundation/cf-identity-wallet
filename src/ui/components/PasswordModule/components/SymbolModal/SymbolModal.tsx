    ...
    >
      ...
      <IonList className="symbol-list">
        <IonItem className="list-header">
          <IonLabel>{i18n.t("createpassword.symbolmodal.symbolcolumn")}</IonLabel>
          <IonLabel>{i18n.t("createpassword.symbolmodal.name")}</IonLabel>
        </IonItem>
        {
          Symbols.map(({ key, labelKey}) => <IonItem className="list-item" key={key}>
            <IonLabel className="symbol">{key}</IonLabel>
            <IonLabel className="symbol-name">{i18n.t(`createpassword.symbolmodal.listsymbol.${labelKey}`)}</IonLabel>
          </IonItem>)
        }
      </IonList>
