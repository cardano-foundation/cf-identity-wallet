...
class ConnectionService extends AgentService {
  ...
  async receiveInvitationFromUrl(url: string): Promise<void> {
    if (url.includes("/oobi")) {
      ...
      if (url.includes("127.0.0.1:3902")) {
        ...
        if (aid && aid.signifyName) {
          ...
          await (
            await fetch("http://127.0.0.1:3001/resolveOobi", {
              method: "POST",
          ...
          await (
            await fetch("http://127.0.0.1:3001/issueAcdcCredential", {
              method: "POST",
              body: JSON.stringify({
                aid: aid.id,
                schemaSaid: SignifyApi.SCHEMA_SAID,
                name: "Jorge Navarro Bendicho",
              }),
              headers: {
                "Content-Type": "application/json",
              },
