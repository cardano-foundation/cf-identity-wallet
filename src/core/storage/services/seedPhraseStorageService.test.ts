...
const addressesMap = new Map([[NetworkType.MAINNET, ["mainnetAddr"]], [NetworkType.TESTNET, ["testnetAddr"]]]);
const rewardAddressesMap = new Map([[NetworkType.MAINNET, ["mainnetRewardAddr"]], [NetworkType.TESTNET, ["testnetRewardAddr"]]]);
...
Addresses.bip32PrivateHexToPublicHex = jest.fn().mockReturnValue(rootExtendedPublicKey);
...
Addresses.convertToRootXPrivateKeyHex = jest.fn().mockReturnValue(rootExtendedPrivateKey);
...
describe("Seed phrase storage service", () => {
  ...
  test("can create a crypto account from identity seed phrase if available", async () => {
    AriesAgent.agent.cryptoAccountIdentitySeedPhraseExists = jest.fn().mockReturnValue(false);
    SecureStorage.get = jest.fn().mockResolvedValue(rootExtendedPrivateKey);
    await seedPhraseSecureStorage.createCryptoAccountFromIdentitySeedPhrase(displayName);
    expect(AriesAgent.agent.cryptoAccountIdentitySeedPhraseExists).toBeCalled();
    expect(SecureStorage.get).toBeCalledWith(KeyStoreKeys.IDENTITY_ROOT_XPRV_KEY);
  ...
  test("should not attempt to create crypto account from identity seed phrase if the agent is not ready", async () => {
    ...
    await expect(seedPhraseSecureStorage.createCryptoAccountFromIdentitySeedPhrase(displayName)).rejects.toThrowError(SeedPhraseStorageService.AGENT_NOT_READY);
  ...
  test("cannot create a crypto account from identity seed phrase if not available", async () => {
    AriesAgent.agent.cryptoAccountIdentitySeedPhraseExists = jest.fn().mockReturnValue(true);
    await expect(seedPhraseSecureStorage.createCryptoAccountFromIdentitySeedPhrase(displayName)).rejects.toThrowError(SeedPhraseStorageService.IDENTITY_SEED_PHRASE_IN_USE);
    expect(AriesAgent.agent.cryptoAccountIdentitySeedPhraseExists).toBeCalled();
    expect(SecureStorage.get).not.toBeCalled();
    expect(AriesAgent.agent.storeCryptoAccountRecord).not.toBeCalled();
  });
  ...
  test("cannot create a crypto account if identity seed phrase is missing", async () => {
    ...
    await expect(seedPhraseSecureStorage.createCryptoAccountFromIdentitySeedPhrase(displayName)).rejects.toThrowError(SeedPhraseStorageService.IDENTITY_ROOT_XPRV_MISSING_OR_MALFORMED);
    expect(SecureStorage.get).toBeCalledWith(KeyStoreKeys.IDENTITY_ROOT_XPRV_KEY);
    expect(AriesAgent.agent.storeCryptoAccountRecord).not.toBeCalled();
  });
  ...
  test("can create a crypto account from a new seed phrase", async () => {
    await seedPhraseSecureStorage.createCryptoAccountFromSeedPhrase(displayName, seedPhrase);
    expect(AriesAgent.agent.storeCryptoAccountRecord).toBeCalledWith(
      rootExtendedPublicKey,
      addressesMap,
      rewardAddressesMap,
    ...
    expect(SecureStorage.set).toBeCalledWith(`${KeyStoreKeys.CRYPTO_ENTROPY_PREFIX}${rootExtendedPublicKey}`, entropy);
    expect(SecureStorage.set).toBeCalledWith(`${KeyStoreKeys.CRYPTO_ROOT_XPRV_KEY_PREFIX}${rootExtendedPublicKey}`, rootExtendedPrivateKey);
  });

  test("should not attempt to create crypto account from new seed phrase if the agent is not ready", async () => {
    AriesAgent.ready = false;
    await expect(seedPhraseSecureStorage.createCryptoAccountFromSeedPhrase(displayName, seedPhrase)).rejects.toThrowError(SeedPhraseStorageService.AGENT_NOT_READY);
    expect(SecureStorage.get).not.toBeCalled();
    expect(AriesAgent.agent.storeCryptoAccountRecord).not.toBeCalled();
  });
  ...
  test("should delete the crypto account from the db if it fails to store items in the secure storage", async () => {
    ...
    await expect(seedPhraseSecureStorage.createCryptoAccountFromSeedPhrase(displayName, seedPhrase)).rejects.toThrowError(errorMsg);
    expect(AriesAgent.agent.storeCryptoAccountRecord).toBeCalledWith(
      rootExtendedPublicKey,
      addressesMap,
      rewardAddressesMap,
    ...
    expect(SecureStorage.set).toBeCalledWith(`${KeyStoreKeys.CRYPTO_ENTROPY_PREFIX}${rootExtendedPublicKey}`, entropy);
    expect(SecureStorage.set).not.toBeCalledWith(`${KeyStoreKeys.CRYPTO_ROOT_XPRV_KEY_PREFIX}${rootExtendedPublicKey}`, rootExtendedPrivateKey);
    expect(AriesAgent.agent.removeCryptoAccountRecordById).toBeCalledWith(rootExtendedPublicKey);
  });
});
