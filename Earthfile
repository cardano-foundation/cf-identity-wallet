  ...
  FOR registry IN $DOCKER_REGISTRIES
    FOR image_tag IN $DOCKER_IMAGES_EXTRA_TAGS
      IF [ "$registry" = "hub.docker.com" ]
        ...
        RUN if [ "$PUSH" = "true" ]; then echo docker push ${HUB_DOCKER_COM_USER}/${DOCKER_IMAGE_NAME}:${image_tag}; fi
      ELSE
        RUN docker tag ${DOCKER_IMAGE_NAME}:latest ${registry}/${DOCKER_IMAGE_NAME}:${image_tag}
        RUN if [ "$PUSH" = "true" ]; then echo docker push ${registry}/${DOCKER_IMAGE_NAME}:${image_tag}; fi
      END
    END
    IF [ "$registry" = "hub.docker.com" ]
      RUN docker tag ${DOCKER_IMAGE_NAME}:latest ${HUB_DOCKER_COM_USER}/${DOCKER_IMAGE_NAME}:${EARTHLY_GIT_SHORT_HASH}
      RUN if [ "$PUSH" = "true" ]; then echo docker push ${HUB_DOCKER_COM_USER}/${DOCKER_IMAGE_NAME}:${EARTHLY_GIT_SHORT_HASH}; fi
    ELSE
      RUN docker tag ${DOCKER_IMAGE_NAME}:latest ${registry}/${DOCKER_IMAGE_NAME}:${EARTHLY_GIT_SHORT_HASH}
      RUN if [ "$PUSH" = "true" ]; then echo docker push ${registry}/${DOCKER_IMAGE_NAME}:${EARTHLY_GIT_SHORT_HASH}; fi
    END
  END
...
idw-keria:
  ...
  IF [ "${REGISTRY_IMAGE_EXISTS}" = "false" ]
    ...
    DO +TEMPLATED_DOCKER_TAG_N_PUSH \
       --PUSH=$PUSH \
       --DOCKER_IMAGE_NAME=${EARTHLY_TARGET_NAME} \
...
idw-witness:
  ...
  IF [ "${REGISTRY_IMAGE_EXISTS}" = "false" ]
    ...
    END
    WAIT
      SAVE IMAGE ${EARTHLY_TARGET_NAME}:keri-${KERI_DOCKER_IMAGE_TAG}
      SAVE IMAGE ${EARTHLY_TARGET_NAME}:latest
    END
    DO +TEMPLATED_DOCKER_TAG_N_PUSH \
       --PUSH=$PUSH \
