...
export class MenuOperationPasswordPasscodeScreen extends PasscodeScreen{
    ...
    async loads() {
        ...
        await expect(this.verifyPasscodeTitle).toHaveText(Passcode.TitleEnterPasscode);
        await expect(this.screenDescriptionText).toBeDisplayed();
        await expect(this.screenDescriptionText).toHaveText(Passcode.DescriptionEnterPasscode);
        for (let i = 0; i < 10; i++) {
